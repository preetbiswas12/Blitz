...
describe("project-id", () => {
  ...
  describe("config file priority", () => {
    test("uses project.id from .kilocodecodecodecodecode/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          // Create config with project ID
          await fs.mkdir(path.join(dir, ".kilocodecodecodecodecode"), { recursive: true })
          await Bun.write(
            path.join(dir, ".kilocodecodecodecodecode", "config.json"),
            JSON.stringify({
              project: {
    ...
    test("falls back to .kilo/config.json when .kilocodecodecodecodecode/config.json is absent", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
    ...
    test("prefers .kilocodecodecodecodecode/config.json over .kilo/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".kilocodecodecodecodecode"), { recursive: true })
          await Bun.write(path.join(dir, ".kilocodecodecodecodecode", "config.json"), JSON.stringify({ project: { id: "new-project" } }))
          await fs.mkdir(path.join(dir, ".kilo"), { recursive: true })
          await Bun.write(
            path.join(dir, ".kilo", "config.json"),
  ...
  describe("fallback behavior", () => {
    ...
    test("handles malformed JSON in config file gracefully", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".kilocodecodecodecodecode"), { recursive: true })
          await Bun.write(path.join(dir, ".kilocodecodecodecodecode", "config.json"), "{ invalid json")
