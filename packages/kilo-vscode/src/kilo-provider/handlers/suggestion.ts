/**
 * Suggestion handlers — extracted from BlitxProvider.
 *
 * Manages suggestion accept and dismiss flows plus recovery after SSE reconnects.
 * No vscode dependency.
 */

import type { KiloClient, SuggestionRequest } from "@blitxcode/sdk/v2/client"
...
export async function handleSuggestionAccept(
  ...
  try {
    ...
    console.error("[Blitx] BlitxProvider: Failed to accept suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function handleSuggestionDismiss(
  ...
  try {
    ...
    console.error("[Blitx] BlitxProvider: Failed to dismiss suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function fetchAndSendPendingSuggestions(ctx: SuggestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Blitx] BlitxProvider: Failed to fetch pending suggestions:", error)
  }
}
