...
const CONFIG_SECTION = "blitx-code.new.autocomplete"
...
export class AutocompleteServiceManager {
  ...
  constructor(context: vscode.ExtensionContext, connectionService: BlitxConnectionService) {
    ...
    this.config = vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("blitx-code.new.language")) {
  ...
  public async showIncompatibilityExtensionPopup() {
    ...
    if (response === disableCopilot) {
      ...
      await vscode.commands.executeCommand("blitx-code.new.autocomplete.disable")
    }
  }
