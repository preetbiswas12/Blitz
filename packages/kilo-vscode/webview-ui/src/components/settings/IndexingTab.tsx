...
import { Button } from "@blitxcode/kilo-ui/button"
import { Card } from "@blitxcode/kilo-ui/card"
import { DEFAULT_VECTOR_STORE } from "@blitxcode/kilo-indexing/config"
import { formatBlitxEmbeddingModelLabel, getBlitxEmbeddingModel } from "@blitxcode/kilo-indexing/embedding-models"
import { Select } from "@blitxcode/kilo-ui/select"
import { Switch } from "@blitxcode/kilo-ui/switch"
import { TextField } from "@blitxcode/kilo-ui/text-field"
import { useConfig } from "../../context/config"
import { formatIndexingLabel, useIndexing } from "../../context/indexing"
import { useBlitxEmbeddingModels } from "../../context/blitx-embedding-models"
...
const IndexingTab: Component = () => {
  ...
  const embeds = useBlitxEmbeddingModels()
  ...
  const blitxDefault = () =>
    getBlitxEmbeddingModel(embeds.catalog().defaultModel, embeds.catalog())?.id ?? embeds.catalog().defaultModel
  const blitxModels = createMemo(() =>
    embeds.catalog().models.map((model) => ({
      value: model.id,
      label: formatBlitxEmbeddingModelLabel(model),
    })),
  )
  const knownBlitxModel = (model: string | null | undefined) =>
    getBlitxEmbeddingModel(model ?? undefined, embeds.catalog())?.id
  const blitxValue = () => knownBlitxModel(cfg().model) ?? blitxDefault()
  const blitxAvailable = () => !!server.profileData() || provider.authStates()[KILO_PROVIDER_ID] !== undefined
  const selectedProvider = () => cfg().provider ?? (blitxAvailable() ? "blitx" : undefined)
  const staleBlitxModel = () => selectedProvider() === "blitx" && !!cfg().model && !knownBlitxModel(cfg().model)
  const providers = createMemo(() =>
    allProviders.filter((item) => item.value !== "blitx" || blitxAvailable() || selectedProvider() === "blitx"),
  )
  const fields = createMemo(() => providerFields(selectedProvider()))

  const saveProvider = (next: ProviderId | undefined) => {
    if (next === "blitx") {
      const model = knownBlitxModel(cfg().model) ?? (blitxDefault() || null)
      updateIndexing({
        provider: next,
        model,
        dimension: null,
  ...
  const saveEnabled = (enabled: boolean) => {
    if (enabled && !cfg().provider && blitxAvailable()) {
      updateIndexing({
        enabled,
        provider: "blitx",
        model: knownBlitxModel(cfg().model) ?? (blitxDefault() || null),
        dimension: null,
      })
      return
    }
  ...
  const saveModel = (value: string) => {
    if (selectedProvider() === "blitx") return
  ...
  const content = (_scope: IndexingScope) => (
    <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
      ...
      <Card>
        ...
        <Show when={selectedProvider() === "blitx"}>
          <Show when={blitxModels().length > 0}>
            <SettingsRow
            ...
            >
              <Select
                options={blitxModels()}
                current={blitxModels().find((item) => item.value === blitxValue())}
                value={(item) => item.value}
                label={(item) => item.label}
                onSelect={(item) => updateIndexing({ model: item?.value ?? blitxDefault(), dimension: null })}
                variant="secondary"
                size="small"
                triggerVariant="settings"
        ...
        <SettingsRow
          ...
          tag={() => (selectedProvider() === "blitx" ? undefined : tag(scope(), [["dimension"]]))}
          last={!selectedProvider() || (fields().length === 0 && !(selectedProvider() === "blitx" && !blitxAvailable()))}
        >
          <TextField
            value={
              staleBlitxModel() || cfg().dimension === undefined || cfg().dimension === null
                ? ""
                : String(cfg().dimension)
            }
            placeholder={
              selectedProvider() === "blitx" ? "Provided by Blitx" : language.t("settings.indexing.dimension.placeholder")
            }
            disabled={selectedProvider() === "blitx"}
            onChange={(value) => saveNumber("dimension", value, { integer: true, min: 1 })}
          />
        </SettingsRow>
        <Show when={selectedProvider() === "blitx" && !blitxAvailable()}>
          <SettingsRow
