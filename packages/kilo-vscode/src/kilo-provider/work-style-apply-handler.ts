...
import type { Config } from "@blitxcode/sdk/v2/client"
...
async function apply(connection: BlitxConnectionService, directory: string, style: WorkStyle) {
  const settings = vscode.workspace.getConfiguration("blitx-code.new")
...
export async function handleWorkStyleApplyMessage(input: {
  ...
  if (input.message.style !== "human-in-the-loop" && input.message.style !== "autonomous") {
    console.error("[Blitx] Invalid style in applyWorkStyle message")
