...
const opencodeClient = process.env.LEGION_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.LEGION_CLIENT
  else process.env.LEGION_CLIENT = opencodeClient
})
