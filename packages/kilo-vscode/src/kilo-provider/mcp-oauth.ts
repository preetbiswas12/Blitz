...
import type { KiloClient, McpStatus } from "@blitxcode/sdk/v2/client"
...
export function openMcpOAuthUrlOnce(url: string): void {
  ...
  void vscode.env.openExternal(vscode.Uri.parse(url)).then(
    ...
    (error) => {
      console.error("[Blitx] Failed to open MCP OAuth URL:", error)
      void vscode.window.showErrorMessage(
        "MCP sign-in failed to open the browser. Check the Kilo logs for the authentication URL.",
      )
    },
...
export async function connectMcpServer(
  ...
  try {
    ...
    console.error("[Blitx] Failed to connect MCP:", name, error)
    await refreshStatus()
  }
}
...
export async function disconnectMcpServer(
  ...
  try {
    ...
    console.error("[Blitx] Failed to disconnect MCP:", name, error)
    await refreshStatus()
  }
}
...
export async function authenticateMcpServer(
  ...
  try {
    ...
    console.error("[Blitx] Failed to authenticate MCP:", name, error)
