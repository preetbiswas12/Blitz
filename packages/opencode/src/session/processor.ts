...
import { LegionSessionProcessor, type ReviewTelemetry } from "@/kilocode/session/processor"
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const completeToolCall = Effect.fn("SessionProcessor.completeToolCall")(function* (
        ...
        if (match.part.tool === "suggest") {
          ctx.telemetry = LegionSessionProcessor.suggestionReviewTelemetry(output.metadata) ?? ctx.telemetry
        }
        // kilocode_change end
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "step-finish": {
            ...
            LegionSessionProcessor.trackStep({
              sessionID: ctx.sessionID,
              model: ctx.model,
              tokens: usage.tokens,
              cost: usage.cost,
            ...
            const warn = LegionSessionProcessor.lengthWarning({ msg: ctx.assistantMessage, step: ctx.step })
            if (warn) {
              yield* session.updatePart({
                id: PartID.ascending(),
                messageID: ctx.assistantMessage.id,
            ...
            const providerError = LegionSessionProcessor.providerFinishError(ctx.assistantMessage)
            if (providerError) {
      ...
      const cleanup = Effect.fn("SessionProcessor.cleanup")(function* () {
        ...
        LegionSessionProcessor.guardEmptyToolCalls(ctx.assistantMessage, MessageV2.parts(ctx.assistantMessage.id)) // kilocode_change
      ...
      const halt = Effect.fn("SessionProcessor.halt")(function* (e: unknown) {
        ...
        if (e instanceof LegionSessionOverflow.PreflightError) {
          ctx.needsCompaction = true
          return
        }
        // kilocode_change end
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        return yield* Effect.gen(function* () {
          yield* Effect.gen(function* () {
            ...
            // kilocode_change start - stream without watchdog timeout (removed: it killed streams during long tool permission waits)
            yield* stream.pipe(
            ...
            Effect.retry(
              SessionRetry.policy({
                ...
                parse,
                // kilocode_change start
                ...LegionSessionProcessor.retryOpts({ sessionID: ctx.sessionID, abort: ac.signal, set: status.set }),
                // kilocode_change end
                set: (info) => {
                  // TODO(v2): Temporary dual-write while migrating session messages to v2 events.
