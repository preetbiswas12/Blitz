...
class UserProfileConfigurableTest : BasePlatformTestCase() {
    ...
    fun `test login updates profile UI`() {
        ...
        edt {
            ...
            buttons(panel).first { it.text == "Login with Blitx Code" }.doClick()
        }
        flush()
    ...
    fun `test logout updates profile UI`() {
        ...
        edt {
            ...
            assertTrue(buttons(panel).any { it.text == "Login with Blitx Code" })
        }
    }
    ...
    fun `test logged out update retains login button`() {
        edt {
            val btn = buttons(panel).first { it.text == "Login with Blitx Code" }
            panel.update(null, KiloAppStatusDto.READY)
            val btn2 = buttons(panel).first { it.text == "Login with Blitx Code" }
            assertSame(btn, btn2)
        }
    }
    ...
    fun `test login shows device auth card before completion`() {
        ...
        flushUntil { text(panel).contains("Sign in to Blitx Code") }

        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Sign in to Blitx Code"))
    ...
    fun `test cancel login invalidates stale completion`() {
        ...
        edt { buttons(panel).first { it.text == "Login with Blitx Code" }.doClick() }
        flushUntil { text(panel).contains("Sign in to Blitx Code") }
    ...
    fun `test preferred focus for logged-out is login button`() {
        edt {
            val focus = panel.preferredFocus()
            val loginBtn = buttons(panel).firstOrNull { it.text == "Login with Blitx Code" }
