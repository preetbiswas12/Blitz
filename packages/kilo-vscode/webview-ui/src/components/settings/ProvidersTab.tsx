...
import { Collapsible } from "@blitxcode/kilo-ui/collapsible"
import { useDialog } from "@blitxcode/kilo-ui/context/dialog"
import { Icon } from "@blitxcode/kilo-ui/icon"
import { ProviderIcon } from "@blitxcode/kilo-ui/provider-icon"
import { Select } from "@blitxcode/kilo-ui/select"
import { Tag } from "@blitxcode/kilo-ui/tag"
...
const ProvidersTab: Component = () => {
  ...
  const blitxLoggedIn = createMemo(() => !!provider.authStates()[KILO_PROVIDER_ID])
  ...
  function connectProvider(item: Provider) {
    if (item.id === KILO_PROVIDER_ID) {
      // Route Blitx Gateway sign-in through the Profile view so the user sees
  ...
  return (
    <div>
      <Show when={!disabledIds().has(KILO_PROVIDER_ID)}>
        {/* Blitx Gateway — always at the top, not editable */}
        <Card>
          <div
            style={{
              display: "flex",
