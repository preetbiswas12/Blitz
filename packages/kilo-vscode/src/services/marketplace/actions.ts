...
export async function removeMarketplaceItemFromAllScopes(
  ...
  try {
    ...
    console.warn("[Blitx] Marketplace removal failed:", err)
    return false
  }
}
...
async function fetchSkills(ctx: MarketplaceActionContext, dir: string) {
  try {
    ...
    console.warn("[Blitx] Failed to fetch CLI skills for marketplace:", err)
    return undefined
  }
}

async function invalidate(
  ctx: { connection: BlitxConnectionService },
  scope: "project" | "global",
  dir: string,
): Promise<void> {
  const client = await ctx.connection.getClientAsync(dir).catch((err: unknown) => {
    console.warn("[Blitx] Marketplace CLI invalidation deferred:", err)
    return null
  })
  if (!client) return
  ...
  if (scope === "global") {
    await client.global.config.update({ config: {} }).catch((err: unknown) => {
      console.warn("[Blitx] global.config.update after marketplace change failed:", err)
    })
  }
  await client.instance.dispose({ directory: dir }).catch((err: unknown) => {
    console.warn("[Blitx] instance.dispose() after marketplace change failed:", err)
  })
}
...
async function removeLegacyMcp(
  ...
  for (const uri of files) {
    ...
    try {
      ...
      console.warn("[Blitx] Failed to remove legacy MCP from", uri.fsPath, err)
    }
  }
  return removed
}
