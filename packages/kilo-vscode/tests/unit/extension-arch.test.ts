...
const KILO_PROVIDER_FILE = path.join(ROOT, "src/BlitxProvider.ts")
...
// ---------------------------------------------------------------------------
// BlitxProvider handler wiring — every new BlitxProvider() must get
...
describe("Extension — BlitxProvider handler wiring", () => {
  const ext = fs.readFileSync(EXTENSION_FILE, "utf-8")

  /**
   * Every `new BlitxProvider(` in extension.ts must be followed (before the
   * next `new BlitxProvider(`) by a `setContinueInWorktreeHandler` call.
   * This prevents future tab/panel additions from silently missing the handler.
   */
  it("every BlitxProvider instance gets setContinueInWorktreeHandler wired", () => {
    const pattern = /new BlitxProvider\(/g
    ...
    expect(
      instances.length,
      "expected at least 3 BlitxProvider instances (sidebar, tab, deserializer)",
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      if (!region.includes("setContinueInWorktreeHandler")) {
        ...
        missing.push(`BlitxProvider at line ${line}`)
      }
    }

    expect(
      missing,
      `These BlitxProvider instances are missing setContinueInWorktreeHandler.\n` +
...
// ---------------------------------------------------------------------------
// BlitxProvider — continueInWorktree error fallback
//
...
describe("BlitxProvider — remote focus lifecycle", () => {
...
describe("BlitxProvider — continueInWorktree error fallback", () => {
