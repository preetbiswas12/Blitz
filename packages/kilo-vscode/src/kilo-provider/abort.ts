import type { KiloClient, SessionStatus } from "@blitxcode/sdk/v2/client"
...
export class SessionAbort {
  ...
  async stop(client: KiloClient, sessionID: string, fallback: string) {
    ...
    if (failures.length > 0) {
      console.error("[Blitx] BlitxProvider: Failed to abort session in one or more directories:", failures)
      return false
    }
