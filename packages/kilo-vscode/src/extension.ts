...
import { BlitxConnectionService } from "./services/cli-backend"
...
export function activate(context: vscode.ExtensionContext) {
  console.log("Blitx extension is now active")
  shuttingDown = false

  const telemetry = TelemetryProxy.getInstance()

  // Create shared connection service (one server for all webviews)
  const connectionService = new BlitxConnectionService(context)
  ...
  const unsubscribeStateChange = connectionService.onStateChange((state) => {
    if (state === "connected") {
      ...
      try {
        ...
        console.log("[Blitx] CLI connected, calling remoteService.refresh()")
        remoteService.refresh().catch((err) => console.warn("[Blitx] initial remote refresh failed:", err))
      } catch {
  ...
  for (const folder of vscode.workspace.workspaceFolders ?? []) {
    void markWorkspace(folder.uri.fsPath, (msg) => console.warn(`[Blitx] ${msg}`))
  }
  ...
  const tabPanels = new Map<vscode.WebviewPanel, BlitxProvider>()
  ...
  const provider = new BlitxProvider(context.extensionUri, connectionService, context)
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(BlitxProvider.viewType, provider, {
  ...
  // commands by default.
  const skip = ["blitx-code.new.agentManagerOpen", "blitx-code.new.agentManager.showTerminal"]
  if (process.platform === "darwin") skip.push("blitx-code.new.agentManager.runScript")
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("blitx-code.new.TabPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        const tabProvider = new BlitxProvider(context.extensionUri, connectionService, context, {
        ...
        panel.onDidDispose(
          () => {
            console.log("[Blitx] Tab panel restored from restart disposed")
  ...
  for (const suffix of settingsViews) {
    context.subscriptions.push(
      vscode.window.registerWebviewPanelSerializer(`blitx-code.new.${suffix}`, {
        deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("blitx-code.new.SubAgentViewerPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("blitx-code.new.sidebarTitle.plusButtonClicked", () => {
      track("new_task", "blitx-code.new.plusButtonClicked")
    }),
    vscode.commands.registerCommand("blitx-code.new.sidebarTitle.historyButtonClicked", () => {
      track("history", "blitx-code.new.historyButtonClicked")
    }),
    vscode.commands.registerCommand("blitx-code.new.sidebarTitle.agentManagerOpen", () => {
      track("agent_manager", "blitx-code.new.agentManagerOpen")
    }),
    vscode.commands.registerCommand("blitx-code.new.sidebarTitle.marketplaceButtonClicked", () => {
      track("marketplace", "blitx-code.new.marketplaceButtonClicked")
    }),
    vscode.commands.registerCommand("blitx-code.new.sidebarTitle.profileButtonClicked", () => {
      track("profile", "blitx-code.new.profileButtonClicked")
    }),
    vscode.commands.registerCommand("blitx-code.new.sidebarTitle.settingsButtonClicked", () => {
      track("settings", "blitx-code.new.settingsButtonClicked")
    }),
    vscode.commands.registerCommand("blitx-code.new.plusButtonClicked", () => {
    ...
    vscode.commands.registerCommand("blitx-code.new.agentManagerOpen", () => {
      agentManagerProvider.openPanel()
    }),
    vscode.commands.registerCommand("blitx-code.new.marketplaceButtonClicked", (directory?: string | null) => {
      marketplacePanelProvider.openPanel(directory)
    }),
    vscode.commands.registerCommand("blitx-code.new.historyButtonClicked", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("blitx-code.new.cycleAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("blitx-code.new.cyclePreviousAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("blitx-code.new.profileButtonClicked", () => {
      settingsEditorProvider.openPanel("profile")
    }),
    vscode.commands.registerCommand("blitx-code.new.settingsButtonClicked", (tab?: string) => {
      settingsEditorProvider.openPanel("settings", tab)
    }),
    vscode.commands.registerCommand("blitx-code.new.openIndexingSettings", () => {
      settingsEditorProvider.openPanel("settings", "indexing")
    }),
    // legacy-migration start
    vscode.commands.registerCommand("blitx-code.new.openMigrationWizard", () => {
      provider.postMessage({ type: "migrationState", needed: true, source: "legacy" })
    }),
    // legacy-migration end
    vscode.commands.registerCommand("blitx-code.new.generateTerminalCommand", async () => {
      ...
      if (!input) return
      await vscode.commands.executeCommand("blitx-code.SidebarProvider.focus")
      await provider.waitForReady()
      provider.postMessage({ type: "triggerTask", text: `Generate a terminal command: ${input}` })
    }),
    vscode.commands.registerCommand("blitx-code.new.toggleRemote", () => {
      remoteService.toggle().catch((err) => console.error("[Blitx] toggleRemote command failed:", err))
    }),
    vscode.commands.registerCommand("blitx-code.new.openInTab", () => {
    ...
    vscode.commands.registerCommand(
      "blitx-code.new.showChanges",
      (arg?: { sessionId?: string; turnId?: string; initialSourceId?: string }) => {
        diffViewerProvider.openFromCommand(arg)
      },
    ),
    vscode.commands.registerCommand("blitx-code.new.openSubAgentViewer", (sessionID: string, title?: string) => {
      subAgentViewerProvider.openPanel(sessionID, title)
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.previousSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionPrevious" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.nextSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionNext" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.previousTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabPrevious" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.nextTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabNext" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.search", () => {
      agentManagerProvider.postMessage({ type: "action", action: "search" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.showTerminal", () => {
    ...
    vscode.commands.registerCommand("blitx-code.new.agentManager.runScript", () => {
      agentManagerProvider.postMessage({ type: "action", action: "runScript" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.toggleDiff", () => {
      agentManagerProvider.postMessage({ type: "action", action: "toggleDiff" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.showShortcuts", () => {
      agentManagerProvider.postMessage({ type: "action", action: "showShortcuts" })
    }),

    vscode.commands.registerCommand("blitx-code.new.agentManager.newTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTab" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.newTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTerminal" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.closeTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeTab" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.newWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newWorktree" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.openWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openWorktree" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.openPR", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openPR" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.closeWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeWorktree" })
    }),
    vscode.commands.registerCommand("blitx-code.new.agentManager.advancedWorktree", () =>
      agentManagerProvider.openAdvancedWorktree(),
    ),
    ...Array.from({ length: 9 }, (_, i) =>
      vscode.commands.registerCommand(`blitx-code.new.agentManager.jumpTo${i + 1}`, () => {
  ...
  context.subscriptions.push(
    vscode.window.registerUriHandler({
      async handleUri(uri: vscode.Uri) {
        ...
        if (sessionId) {
          console.log("[Blitx] URI handler: opening cloud session:", sessionId)
        ...
        console.log("[Blitx] URI handler: applying linked Kilo selection:", { modelID, agent })
        await vscode.commands.executeCommand(`${BlitxProvider.viewType}.focus`)
...
async function openKiloInNewTab(
  ...
  connectionService: BlitxConnectionService,
  agentManagerProvider: AgentManagerProvider,
  tabPanels: Map<vscode.WebviewPanel, BlitxProvider>,
  ...
  const panel = vscode.window.createWebviewPanel("blitx-code.new.TabPanel", EXTENSION_DISPLAY_NAME, targetCol, {
    enableScripts: true,
  ...
  panel.onDidDispose(
    () => {
      console.log("[Blitx] Tab panel disposed")
      tabPanels.delete(panel)
      tabProvider.dispose()
    },
    null,
