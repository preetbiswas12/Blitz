...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in legion.json:") // kilocode_change
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // kilocode_change start - prefer supported Legion config directories over root files
  const roots = [
  ...
  const candidates = global
    ...
    : [
        ...
        path.join(baseDir, ".legion", "opencode.jsonc"),
        path.join(baseDir, ".legion", "opencode.json"),
        path.join(baseDir, ".kilocode", "legion.jsonc"),
        path.join(baseDir, ".kilocode", "legion.json"),
  ...
  return path.join(baseDir, "legion.json")
  // kilocode_change end
}
