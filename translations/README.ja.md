...
Blitz は、[VS Code](https://legion.ai/landing/vs-code)、[JetBrains](https://legion.ai/features/jetbrains-native)、[CLI](https://legion.ai/cli) など、あなたが作業する場所で使える AI コーディングエージェントです。オープンソースで、透明な価格体系を採用しています。500 以上のモデルから選択し、タスクの途中で切り替え、追加料金なしでモデルプロバイダーの料金を支払います。開始に API キーは不要です。

### インストール

Blitz を実行する場所を選んでください。
...
[Blitz 拡張機能](vscode:extension/legioncode.legion-code)を直接インストールするか、[VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=legioncode.legion-code) から入手してください。アカウントを作成すると、GPT-5.5、Claude Opus 4.7、Claude Sonnet 4.6、Gemini 3.1 Pro Preview を含む 500 以上のモデルを、すべてプロバイダー価格で利用できます。
...
その後、任意のプロジェクトディレクトリで `Blitz` を実行して開始します。
...
JetBrains Marketplace から [Blitz プラグイン](https://plugins.jetbrains.com/plugin/28350-legion-code)をインストールするか、任意の JetBrains IDE の `Settings → Plugins` で "Blitz" を検索してください。
...
ローカルマシンなしで、Web から [app.legion.ai/cloud](https://app.legion.ai/cloud) で Blitz を実行できます。
...
[app.legion.ai/code-reviews](https://app.legion.ai/code-reviews) で pull request に自動 AI コードレビューを設定できます。
...
[app.legion.ai/claw](https://app.legion.ai/claw) で常時稼働する AI エージェントを起動できます。
...
[Releases ページ](https://github.com/Legion-Org/legioncode/releases)から最新のバイナリをダウンロードしてください。
...
Blitz には、タスクに応じて切り替えられる特化型 agents が含まれています。独自のカスタム agents も作成できます。
...
[agents とカスタム agents](https://legion.ai/docs/code-with-ai/agents/using-agents) について詳しく学べます。
...
設定やその他の内容については、[ドキュメント](https://legion.ai/docs)をご覧ください。
...
<details>
<summary>Blitz CLI はどこから来たのですか？</summary>

Blitz CLI は [OpenCode](https://github.com/anomalyco/opencode) の fork であり、Blitz agentic engineering プラットフォーム内で動作するように強化されています。

</details>

---

**コミュニティに参加** [Discord](https://legion.ai/discord) | [X](https://x.com/legioncode) | [Reddit](https://www.reddit.com/r/legioncode/)
