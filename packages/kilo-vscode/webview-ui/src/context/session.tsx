...
import { showToast } from "@blitxcode/kilo-ui/toast"
import type {
  SessionInfo,
  SessionModelUsage,
  SessionUpdate,
...
export const SessionProvider: ParentComponent = (props) => {
  ...
  createEffect(() => {
    ...
    if (pending.modelID && !provider.providers()[KILO_PROVIDER_ID]?.models[pending.modelID]) {
      console.warn("[Blitx] Ignoring unavailable Blitx catalog model:", pending.modelID)
      return
    }
    if (pending.agent && !agentNames().has(pending.agent)) {
      console.warn("[Blitx] Ignoring unavailable Blitx agent:", pending.agent)
      return
    }
  ...
  function handleExtensionMessage(message: ExtensionMessage): void {
    ...
    switch (message.type) {
      ...
      case "cloudSessionImportFailed":
        ...
        console.error("[Blitx] Cloud session import failed:", message.error)
        break
  ...
  function handlePartUpdated(
    ...
    if (!effectiveMessageID) {
      console.warn("[Blitx] Part updated without messageID:", part.id, part.type)
      return
    }
  ...
  function sendMessage(
    ...
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot send message: not connected")
      return
    }
  ...
  function sendCommand(
    ...
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot send command: not connected")
      return
    }
  ...
  function abort() {
    ...
    if (!scope) {
      console.warn("[Blitx] Cannot abort: no current or pending session")
      return
    }
  ...
  function compact() {
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot compact: not connected")
      return
    }

    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Blitx] Cannot compact: no current session")
      return
    }
  ...
  function createSession() {
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot create session: not connected")
      return
    }
  ...
  function loadSessions() {
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot load sessions: not connected")
      return
    }
  ...
  function selectSession(id: string) {
    ...
    if (id.startsWith("cloud:")) {
      console.warn("[Blitx] Cannot select cloud preview session via selectSession")
      return
    }
  ...
  function selectCloudSession(cloudSessionId: string) {
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot select cloud session: not connected")
      return
    }
    const key = `cloud:${cloudSessionId}`
  ...
  function deleteSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot delete session: not connected")
      return
    }
  ...
  function renameSession(id: string, title: string) {
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot rename session: not connected")
      return
    }
  ...
  function exportSessionTranscript(id: string) {
    if (!server.isConnected()) {
      console.warn("[Blitx] Cannot export session transcript: not connected")
      return
    }
    if (id.startsWith("cloud:")) {
      console.warn("[Blitx] Cannot export cloud session transcript")
      return
    }
