...
export class BrowserAutomationService implements vscode.Disposable {
  ...
  constructor(private readonly connectionService: BlitxConnectionService) {
    // Listen for settings changes
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("blitx-code.new.browserAutomation")) {
  ...
  private async register(): Promise<void> {
    ...
    if (!client) {
      console.error("[Blitx] BrowserAutomationService: No SDK client available")
    ...
    try {
      ...
      if (serverStatus?.status === "connected") {
        ...
        console.error(
          "[Blitx] BrowserAutomationService: MCP server failed:",
      ...
      console.error("[Blitx] BrowserAutomationService: Failed to register MCP server:", error)
      this.setState("failed")
    }
  }
  ...
  private async unregister(): Promise<void> {
    ...
    if (client) {
      try {
        ...
        console.error("[Blitx] BrowserAutomationService: Failed to disconnect MCP server:", error)
      }
    }
  ...
  private setState(state: BrowserAutomationState): void {
    ...
    console.log(`[Blitx] BrowserAutomationService: State ${this.state} → ${state}`)
    this.state = state
  }
