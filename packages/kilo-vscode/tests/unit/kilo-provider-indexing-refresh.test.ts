...
const { BlitxProvider } = await import("../../src/BlitxProvider")
...
describe("BlitxProvider indexing refresh", () => {
  it("reloadAfterAuthChange fetches config first, then indexing status", async () => {
    const provider = new BlitxProvider({} as never, {} as never)
  ...
  it("handleUpdateConfig no longer eagerly fetches indexing status", async () => {
    ...
    const provider = new BlitxProvider({} as never, conn.service as never)
    const internal = provider as unknown as Internals
  ...
  it("fetchAndSendIndexingStatus uses current session directory header", async () => {
    ...
    try {
      const provider = new BlitxProvider(
        {} as never,
        {
          getClient: () => ({}) as never,
