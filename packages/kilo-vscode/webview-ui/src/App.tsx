...
import { DataProvider } from "@blitxcode/kilo-ui/context/data"
...
import { BlitxEmbeddingModelsProvider } from "./context/blitx-embedding-models"
import type { Message as SDKMessage, Part as SDKPart } from "@blitxcode/sdk/v2"
  ...
  onMount(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (message?.type === "action" && message.action) {
        console.log("[Blitx] App: 🎬 action:", message.action)
        handleViewAction(message.action)
      }
      if (message?.type === "navigate" && message.view && VALID_VIEWS.has(message.view)) {
        console.log("[Blitx] App: 🧭 navigate:", message.view, message.tab ? `tab=${message.tab}` : "")
      ...
      if (message?.type === "openCloudSession" && message.sessionId) {
        console.log("[Blitx] App: ☁️ openCloudSession:", message.sessionId)
      ...
      if (message?.type === "viewSubAgentSession" && message.sessionID) {
        console.log("[Blitx] App: 🔍 viewSubAgentSession:", message.sessionID)
      ...
      if (message?.type === "migrationState") {
        console.log("[Blitx] App: 🔄 migrationState:", message.needed)
