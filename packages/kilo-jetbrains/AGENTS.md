...
- A random 32-byte hex password is passed via `BLITX_SERVER_PASSWORD` env var for Basic Auth.
- Fixed env vars set on every spawn: `BLITX_CLIENT=jetbrains`, `BLITX_PLATFORM=jetbrains`, `KILO_APP_NAME=kilo-code`, `KILO_ENABLE_QUESTION_TOOL=true`, `BLITX_DISABLE_CLAUDE_CODE=true`, `KILOCODE_FEATURE=jetbrains-plugin`.
...
- **Via Turbo**: `bun turbo build --filter=@blitxcode/kilo-jetbrains` from repo root.
