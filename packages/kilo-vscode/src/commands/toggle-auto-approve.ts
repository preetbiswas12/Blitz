...
import type { Event, KiloClient } from "@blitxcode/sdk/v2/client"
import type { BlitxConnectionService } from "../services/cli-backend/connection-service"
...
const CONFIG = "blitx-code.new.autoApprove"
const KEY = "enabled"
...
export function registerToggleAutoApprove(
      ...
      try {
        ...
        for (const req of pending) {
          ...
          await client.permission
            ...
            .catch((err) => {
              console.error("[Blitx] toggleAutoApprove: failed to drain pending:", err)
            })
        }
      } catch (err) {
        console.error("[Blitx] toggleAutoApprove: failed to list pending permissions:", err)
      }
    }
  ...
  const approve = async (event: Asked, directory?: string) => {
    ...
    return client.permission
      ...
      .then(
        () => true,
        (err) => {
          console.error("[Blitx] toggleAutoApprove: failed to auto-reply:", err)
          return false
        },
      )
  }
  ...
  context.subscriptions.push(vscode.commands.registerCommand("blitx-code.new.toggleAutoApprove", toggle))
...
function tryGetClient(connectionService: BlitxConnectionService): KiloClient | undefined {
  try {
