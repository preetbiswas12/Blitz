 ...
 * 2. Every WebviewMessage type handled in BlitxProvider has a corresponding member in the WebviewMessage union
...
describe("BlitxProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in BlitxProvider exist in WebviewMessage union", () => {
    ...
    expect(
      unrecognized,
      `BlitxProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
