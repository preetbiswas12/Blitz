...
function getConfig() {
  return vscode.workspace.getConfiguration("blitx-code.new")
}
...
export function watchWorkStyleConfig(post: (message: unknown) => void, next?: vscode.Disposable) {
  ...
  const watcher = vscode.workspace.onDidChangeConfiguration((event) => {
    if (keys.some((key) => event.affectsConfiguration(`blitx-code.new.${key}`))) post(getWorkStylePayload())
  })
...
async function hasAnySession(connection: BlitxConnectionService, directory: string): Promise<boolean> {
...
async function initializeWorkStyle(connection: BlitxConnectionService, directory: string): Promise<void> {
...
export async function handleWorkStyleMessage(input: {
  ...
  connection: BlitxConnectionService
  directory: string
  ...
  if (input.message.type === "requestWorkStyle") {
    const initialized = await initializeWorkStyle(input.connection, input.directory)
      .then(() => true)
      .catch((err: unknown) => {
        console.error("[Blitx] Failed to initialize work style:", err)
        return false
      })
  ...
  if (!input.message.style) {
    console.error("[Blitx] Missing style in setWorkStyle message")
    return true
  }
