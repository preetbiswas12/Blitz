...
import { BlitxSessionCompaction } from "@/kilocode/session/compaction"
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      BlitxSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => BlitxSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const result = await Effect.runPromise(
      BlitxSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          BlitxSessionPromptQueue.retarget(sessionID, injected)
          return {
            active: BlitxSessionPromptQueue.active(sessionID),
            ids: BlitxSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id),
          }
        }),
        ...
        const result = await Effect.runPromise(
          BlitxSessionPromptQueue.enqueue(
            ...
            Effect.promise(async () => {
              ...
              return { compact, ids: BlitxSessionPromptQueue.scope(session.id, messages).map((item) => item.info.id) }
            }),
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      BlitxSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: BlitxSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: BlitxSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      BlitxSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: BlitxSessionPromptQueue.hasFollowup(sessionID) })
    ...
    const third = Effect.runPromise(
      BlitxSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: BlitxSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(BlitxSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
  ...
  test("cancel on a session with no active tail is a no-op and does not leak state", async () => {
    ...
    await Effect.runPromise(BlitxSessionPromptQueue.cancel(sessionID))

    expect(BlitxSessionPromptQueue._hasInternalState(sessionID)).toBe(false)
          ...
          scoped(tmp.path, async (prompt) => {
            ...
            const ids = await Effect.runPromise(
              BlitxSessionPromptQueue.enqueue(
                session.id,
                MessageID.make("msg_probe"),
                Effect.succeed(BlitxSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
                Effect.succeed([]),
              ),
            )
            expect(ids).toEqual([])
            expect(BlitxSessionPromptQueue.hasFollowup(session.id)).toBe(false)
          }),
      })
    } finally {
      server.stop(true)
