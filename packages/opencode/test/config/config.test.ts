...
const schemaConfig = (config: object) => ({ $schema: "https://preetbiswas12.github.io/Blitz/config.json", ...config }) // kilocode_change
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      expect(content).toContain('"$schema": "https://preetbiswas12.github.io/Blitz/config.json"') // kilocode_change
...
it.instance("jsonc overrides json in the same directory", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://preetbiswas12.github.io/Blitz/config.json", // kilocode_change
        model: "base",
        username: "base",
      },
