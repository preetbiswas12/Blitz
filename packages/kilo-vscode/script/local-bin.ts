...
/**
 * Ensures the VS Code extension has a CLI binary at `packages/kilo-vscode/bin/blitx`.
 *
 * Strategy:
 * 1) If `bin/blitx` already exists -> ok.
 ...
 * 4) Copy the resulting binary into `packages/kilo-vscode/bin/blitx` and chmod +x.
 *
...
const binName = process.platform === "win32" ? "blitx.exe" : "blitx"
...
async function cliSourceHash(): Promise<string | null> {
  try {
    ...
    return `${opencodeResult.text().trim()}-${coreResult.text().trim()}-${indexingResult.text().trim()}-${sandboxResult.text().trim()}`
  } catch {
    return null
  }
}
...
async function findBlitxBinaryInOpencodeDist(): Promise<string | null> {
  ...
  const preferred = join(distDir, `@blitxcode`, tag, "bin", binName)
  try {
    statSync(preferred)
  ...
  // Fallback: find any dist/**/bin/blitx or blitx.exe
  const queue = [distDir]
  while (queue.length) {
    const dir = queue.pop()
    if (!dir) continue
    ...
    for (const e of entries) {
      ...
      if (e.isFile() && (e.name === "blitx" || e.name === "blitx.exe") && basename(dirname(p)) === "bin") {
...
async function ensureBuiltBinary(): Promise<string> {
  const found = await findBlitxBinaryInOpencodeDist()
  ...
  const built = await findBlitxBinaryInOpencodeDist()
  if (!built) {
    throw new Error(
      `CLI build completed but no binary was found in ${join(opencodeDir, "dist")} (expected dist/**/bin/blitx).`,
    )
  }
  return built
}
