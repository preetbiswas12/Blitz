...
const PANEL_TITLES: Record<PanelView, string> = {
  settings: "Blitx Settings",
  profile: "Blitx Profile",
 ...
 *
 * Uses a full BlitxProvider under the hood so each panel has
...
export class SettingsEditorProvider implements vscode.Disposable {
  ...
  private providers = new Map<PanelView, BlitxProvider>()
  ...
  /** Extract the PanelView from a viewType string like "blitx-code.new.settingsPanel". */
  ...
  private wirePanel(panel: vscode.WebviewPanel, view: PanelView, projectDirectory: string | null): void {
    ...
    // Create a dedicated BlitxProvider for this panel so it has full
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "webviewReady") {
        // Small delay to let BlitxProvider's own webviewReady handler finish first
        setTimeout(() => {
    ...
    panel.onDidDispose(() => {
      console.log(`[Blitx] ${title} panel disposed`)
