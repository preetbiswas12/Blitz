...
Actual description here.`
  ...
  describe("discoverWorkflows", () => {
    test("discovers project workflows", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".kilocodecodecode", "workflows")
    ...
    test("discovers workflows from legacy .kilocodecodecodecode/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".kilocodecodecodecode", "workflows")
    ...
    test("discovers global workflows from ~/.kilocodecodecode/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".kilocodecodecode", "workflows", "global.md"), "# Global\n\nGlobal workflow")
      ...
      expect(
        workflows.some((w) => w.source === "global" && w.path.includes(path.join(".kilocodecodecode", "workflows", "global.md"))),
      ).toBe(true)
    })

    test("applies markdown substitutions to workflow content", async () => {
      process.env.kilocodecodecode_WORKFLOW_TEST = "env content"
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".kilocodecodecode", "workflows")
      ...
      try {
        ...
        delete process.env.kilocodecodecode_WORKFLOW_TEST
      }
    })
  })
  ...
  describe("convertToCommand", () => {
    test("converts workflow to command format", () => {
      const workflow: WorkflowsMigrator.kilocodecodecodecodeWorkflow = {
        name: "code-review",
    ...
    test("project workflows override global workflows with same name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const projectDir = path.join(dir, ".kilocodecodecode", "workflows")
