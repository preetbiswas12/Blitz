...
describe("prompt.ts Kilo-specific invariants", () => {
  ...
  test("dismissAll for suggestions and questions runs before enqueue, without cancelling the in-flight fiber", () => {
    ...
    // Critically, the block must NOT call state.cancel or BlitxSessionPromptQueue.reserve —
    ...
    const block = content.match(
      /kilocode_change start[^\n]*unblock tools[\s\S]*?Suggestion\.dismissAll[\s\S]*?question\.dismissAll[\s\S]*?BlitxSessionPromptQueue\.enqueue/,
    )
    expect(block).not.toBeNull()
    expect(content).not.toMatch(/state\.cancel\(input\.sessionID\)/)
    expect(content).not.toMatch(/BlitxSessionPromptQueue\.reserve/)
  })
  ...
  test("runLoop breaks out between LLM steps when a newer prompt was enqueued", () => {
    ...
    expect(content).toContain("BlitxSessionPromptQueue.hasFollowup(sessionID)")
  })
})
