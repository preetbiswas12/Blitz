...
import type { BlitxProvider } from "../../BlitxProvider"
...
export function registerCodeActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("blitx-code.new.explainCode", async () => {
    ...
    vscode.commands.registerCommand("blitx-code.new.fixCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("blitx-code.new.improveCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("blitx-code.new.addToContext", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("blitx-code.new.focusChatInput", async () => {
      const view = target()
      if (view === provider) {
        await reveal()
      }
