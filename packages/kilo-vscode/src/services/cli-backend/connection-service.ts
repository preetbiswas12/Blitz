...
import { createKiloClient, type KiloClient } from "@blitxcode/sdk/v2/client"
 ...
 * Multiple BlitxProvider instances subscribe to it for SSE events and state changes.
 */
export class BlitxConnectionService {
  ...
  /**
   * Subscribe to notification dismiss events broadcast from any BlitxProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a notification dismiss event to all subscribed BlitxProvider instances.
   */
  ...
  /**
   * Subscribe to language change events broadcast from any BlitxProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a language change event to all subscribed BlitxProvider instances.
   */
  ...
  /**
   * Subscribe to profile change events broadcast from any BlitxProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a profile change event to all subscribed BlitxProvider instances.
   */
  ...
  /**
   * Subscribe to migration-complete events broadcast from any BlitxProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a migration-complete event to all subscribed BlitxProvider instances.
   */
  ...
  /**
   * Subscribe to favorites change events broadcast from any BlitxProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a favorites change event to all subscribed BlitxProvider instances.
   */
   ...
   * BlitxProvider (root + worktree dirs). Used by drainPendingPrompts() to
   ...
   * directory known to any currently-mounted BlitxProvider.
   *
  ...
  private sendViewed(): void {
    ...
    void this.client.session
      ...
      .catch((err) => console.warn("[Blitx] ConnectionService: viewed flush failed:", err))
      .finally(() => {
  ...
  private startHealthPoll(baseUrl: string, password: string): void {
    ...
    this.healthPollTimer = setInterval(async () => {
      ...
      if (!healthy && this.state === "connected") {
        console.warn("[Blitx] ConnectionService: ❤️‍🩹 Health check failed — forcing SSE reconnect")
  ...
  private handleServerExit(code: number | null): void {
    console.warn("[Blitx] ConnectionService: CLI background process exited:", code)
