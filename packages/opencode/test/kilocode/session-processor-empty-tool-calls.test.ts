        ...
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === LegionSessionProcessor.REASONING_LENGTH_WARNING,
          )

          expect(warning?.ignored).toBe(true)

          const modelMsgs = yield* MessageV2.toModelMessagesEffect([{ info: handle.message, parts }], mdl)
          expect(JSON.stringify(modelMsgs)).not.toContain(LegionSessionProcessor.REASONING_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === LegionSessionProcessor.OUTPUT_LENGTH_WARNING,
          )
          ...
          expect(json).not.toContain(LegionSessionProcessor.OUTPUT_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
