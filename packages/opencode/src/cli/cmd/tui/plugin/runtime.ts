...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    // kilocode_change start - install local themes into supported Legion config directories
    const base = path.basename(source_dir)
    const local_dir =
      base === ".legion" || base === ".kilocode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".legion", "themes")
    // kilocode_change end
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".legion", "tui.json"), // kilocode_change
  }
}
