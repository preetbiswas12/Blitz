 ...
 *
 * Reads legacy Blitx v5.x data from VS Code SecretStorage and the extension's
...
export async function clearLegacyData(context: vscode.ExtensionContext): Promise<void> {
  ...
  const legacyStateKeys = [
    "blitx-code.allowedCommands",
    "blitx-code.deniedCommands",
    "blitx-code.autoApprovalEnabled",
    "blitx-code.fuzzyMatchThreshold",
    "blitx-code.diffEnabled",
    "blitx-code.language",
    "blitx-code.customModes",
    "blitx-code.firstInstallCompleted",
    "blitx-code.telemetrySetting",
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("blitx-code.new.autocomplete")
...
function readLegacySettings(context: vscode.ExtensionContext): LegacySettings {
  ...
  return {
    autoApprovalEnabled: context.globalState.get<boolean>("blitx-code.autoApprovalEnabled"),
    allowedCommands: context.globalState.get<string[]>("blitx-code.allowedCommands"),
    deniedCommands: context.globalState.get<string[]>("blitx-code.deniedCommands"),
    ...
    language: context.globalState.get<string>("blitx-code.language"),
