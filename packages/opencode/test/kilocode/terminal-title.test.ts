...
describe("KiloTerminalTitle", () => {
  test("format_noneStyle_hidesStatusIcon", () => {
    expect(KiloTerminalTitle.format({ base, title: "Build status", indicator: "working" })).toBe(
      "Legion CLI | Build status",
    )
  })

  test("format_noIndicator_returnsBaseTitle", () => {
    expect(KiloTerminalTitle.format({ base, indicator: "none", icon: "unicode" })).toBe("Legion CLI")
  })

  test("format_unicodeStyle_usesUnicodeIcons", () => {
    expect(KiloTerminalTitle.format({ base, indicator: "working", icon: "unicode" })).toBe("◔ Legion CLI")
    expect(KiloTerminalTitle.format({ base, indicator: "attention", icon: "unicode" })).toBe("⚠ Legion CLI")
    expect(KiloTerminalTitle.format({ base, indicator: "finished", icon: "unicode" })).toBe("✓ Legion CLI")
  })

  test("format_emojiStyle_usesEmojiIcons", () => {
    expect(KiloTerminalTitle.format({ base, indicator: "working", icon: "emojis" })).toBe("💭 Legion CLI")
    expect(KiloTerminalTitle.format({ base, indicator: "attention", icon: "emojis" })).toBe("🔶 Legion CLI")
    expect(KiloTerminalTitle.format({ base, indicator: "finished", icon: "emojis" })).toBe("✅ Legion CLI")
  })
  ...
  test("format_longSessionTitle_truncatesToExistingLimit", () => {
    ...
    ).toBe("◔ Legion CLI | 1234567890123456789012345678901234567...")
  })
  ...
  test("session_newIdleSession_hasNoIndicator", () => {
    ...
    ).toEqual({ title: "Legion CLI | Build status", id: "parent", active: false, indicator: "none" })
  })
  ...
  test("session_busySession_isWorking", () => {
    ...
    ).toEqual({ title: "◔ Legion CLI | Build status", id: "parent", active: true, indicator: "working" })
  })
