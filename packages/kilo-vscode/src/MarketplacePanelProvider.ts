...
import type { GlobalEvent, SessionStatus } from "@blitxcode/sdk/v2/client"
...
export class MarketplacePanelProvider implements vscode.Disposable {
  public static readonly viewType = "blitx-code.new.marketplacePanel"
  ...
  constructor(
    ...
    private readonly connection: BlitxConnectionService,
  ...
  private async sync(reconcile: boolean): Promise<void> {
    ...
    try {
      ...
      console.warn("[Blitx] Marketplace session status sync failed:", err)
    }
  }
  ...
  private async fetchData(): Promise<void> {
    ...
    try {
      ...
      console.warn("[Blitx] Marketplace data fetch failed:", err)
      this.post({
  ...
  private post(msg: unknown): void {
    ...
    void this.panel.webview.postMessage(msg).then(undefined, (err) => {
      console.warn("[Blitx] Marketplace panel postMessage failed:", err)
    })
  }
