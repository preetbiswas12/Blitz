...
export class DiffViewerProvider implements vscode.Disposable {
  public static readonly viewType = "blitx-code.new.DiffViewerPanel"
  ...
  constructor(
    ...
    this.output = vscode.window.createOutputChannel("Blitx Diff Panel")
  }
  ...
  /**
   * Entry point for the `blitx-code.new.showChanges` command. Composes the
  ...
  private onWebviewReady(): void {
    ...
    void this.panel.webview.postMessage({
      ...
      languageOverride: vscode.workspace.getConfiguration("blitx-code.new").get<string>("language"),
