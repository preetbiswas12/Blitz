...
export async function stopSpeechCapture(requestId: string): Promise<Audio> {
  ...
  const size = await stat(state.file)
    .then((info) => info.size)
    .catch((err: unknown) => {
      console.warn("[Blitx] Failed to stat speech recording", err)
      return 0
    })
...
async function findFFmpeg(): Promise<string> {
  ...
  for (const bin of paths) {
    ...
    try {
      ...
      console.warn(`[Blitx] FFmpeg candidate failed: ${bin}`, err)
    }
  }

  throw new Error("Speech input needs the bundled FFmpeg helper, but it was not found. Rebuild or reinstall Blitx.")
}
...
async function removeFile(file: string): Promise<void> {
  await unlink(file).catch((err: unknown) => {
    console.warn("[Blitx] Failed to remove speech recording", err)
  })
}
