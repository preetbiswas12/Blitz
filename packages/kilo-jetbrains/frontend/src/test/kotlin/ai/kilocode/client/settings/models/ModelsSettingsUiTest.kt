...
class ModelsSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test failed save after dispose shows notification`() {
        ...
        flushUntil {
            notes.any { it.groupId == "Blitx Code" && it.type == NotificationType.ERROR }
        }
    ...
    fun `test logged out save keeps login banner stable`() {
        ...
        edt { ui = ModelsSettingsUi(uiScope, app, workspaces, directory = "/test") }
        val panel = requireUi()
        flushUntil { text(panel).contains("Old") && text(panel).contains("Sign in to Blitx Code") }
        ...
        edt {
            select(panel, "new")
            panel.applyDraft()
            assertTrue(text(panel).contains("Sign in to Blitx Code"))
