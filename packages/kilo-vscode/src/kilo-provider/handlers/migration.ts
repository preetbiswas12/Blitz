/**
 * Legacy migration handlers — extracted from BlitxProvider.
 *
 * Manages the migration wizard for users upgrading from Blitx v5.x.
...
export async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[Blitx] BlitxProvider: 🔄 Legacy data detected, showing migration wizard")
...
async function startLegacyMigration(
  ...
  try {
    ...
    console.error("[Blitx] BlitxProvider: ❌ Migration failed", error)
    ctx.postMessage({
      type: "migrationComplete",
      source: "legacy",
      operationId,
