...
import type { BlitxConnectionService } from "../cli-backend/connection-service"
...
export function registerCommitMessageService(
  ...
  const command = vscode.commands.registerCommand(
    "blitx-code.new.generateCommitMessage",
    async (arg?: vscode.SourceControl) => {
      ...
      try {
        ...
        console.error("[Blitx] Failed to connect to Blitx backend:", err)
        vscode.window.showErrorMessage("Failed to connect to Blitx backend. Please try again.")
        return
      }
      ...
      await vscode.window
        .withProgress(
          ...
          async (_progress, token) => {
            ...
            try {
              ...
              console.log("[Blitx] Commit message generated successfully")
            } finally {
              clearTimeout(timer)
            }
          },
        ...
        .then(undefined, (error: unknown) => {
          if (userCancelled) {
            console.log("[Blitx] Commit message generation was cancelled by user")
            return
          }
          if (timedOut) {
            console.log("[Blitx] Commit message generation timed out")
            vscode.window.showErrorMessage("Commit message generation timed out. Please try again.")
            return
          }
          const msg = getErrorMessage(error)
          console.error("[Blitx] Failed to generate commit message:", msg)
