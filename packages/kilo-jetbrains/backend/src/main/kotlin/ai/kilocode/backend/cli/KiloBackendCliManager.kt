...
/**
 * Manages the Blitx CLI binary lifecycle.
 *
...
class KiloBackendCliManager(
    ...
    private suspend fun spawn(cli: File): CliServer.State =
        withContext(Dispatchers.IO) {
            ...
            log.info("CLI env: BLITX_CLIENT=jetbrains BLITX_PLATFORM=jetbrains KILO_APP_NAME=kilo-code")
            val proc = try {
                builder.start()
            } catch (e: Exception) {
...
internal fun buildKiloCliEnv(
    ...
    putAll(base)
    put("BLITX_SERVER_PASSWORD", pwd)
    put("BLITX_CLIENT", "jetbrains")
    put("KILO_ENABLE_QUESTION_TOOL", "true")
    put("BLITX_PLATFORM", "jetbrains")
    put("KILO_APP_NAME", "kilo-code")
    put("KILO_TELEMETRY_LEVEL", if (KiloDevMode.enabled()) "off" else "all")
    if (!KiloClaudeCompatSettings.get()) put("BLITX_DISABLE_CLAUDE_CODE", "true")
