...
function safeStringify(value: unknown): string | undefined {
  try {
    ...
    console.warn("[Blitx] getErrorMessage: JSON.stringify failed", err)
  }
  return undefined
}
...
export async function runWithMessageConfirmation<T>(
  ...
  try {
    ...
    if (await state.wait(id)) {
      console.warn(`[Blitx] ${label} ended after server accepted it; ignoring transport error`, {
...
/**
 * Shared interface for the subset of BlitxProvider state needed by session-refresh helpers.
 * Extracted here so the logic can be tested without importing BlitxProvider (and vscode).
 */
