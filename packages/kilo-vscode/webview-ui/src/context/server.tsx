...
export const ServerProvider: ParentComponent = (props) => {
  ...
  onMount(() => {
    const unsubscribe = vscode.onMessage((message: ExtensionMessage) => {
      switch (message.type) {
        case "ready":
          console.log("[Blitx] Server ready:", message.serverInfo)
        ...
        case "connectionState":
          console.log("[Blitx] Connection state changed:", message.state)
        ...
        case "error":
          console.error("[Blitx] Server error:", message.message)
          setErrorMessage(message.message)
          setErrorDetails(message.message)
          break

        case "profileData":
          console.log("[Blitx] Profile data:", message.data ? "received" : "null")
          setProfileData(message.data)
          break

        case "deviceAuthStarted":
          console.log("[Blitx] Device auth started")
          setDeviceAuth({
            status: "pending",
            code: message.code,
        ...
        case "deviceAuthComplete":
          console.log("[Blitx] Device auth complete")
        ...
        case "deviceAuthFailed":
          console.log("[Blitx] Device auth failed:", message.error)
          setDeviceAuth({ status: "error", error: message.error })
          break

        case "deviceAuthCancelled":
          console.log("[Blitx] Device auth cancelled")
    ...
    console.log("[Blitx] Webview ready")
   ...
   * outside the Profile page — e.g. the Blitx Gateway card in the Providers
