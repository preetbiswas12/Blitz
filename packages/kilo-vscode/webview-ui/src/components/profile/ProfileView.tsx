...
const ProfileView: Component<ProfileViewProps> = (props) => {
  ...
  const handleGetPass = () => {
    vscode.postMessage({ type: "openExternal", url: "https://blitx.ai/pricing/kilo-pass" })
  }
              ...
              <Show when={data().balance}>
                {(balance) => (
                  <Card style={{ display: "flex", "flex-direction": "column", gap: "12px" }}>
                    ...
                    {/* Blitx Pass is part of personal credits, so only show it on the personal account */}
                    <Show when={(data().currentOrgId ?? null) === null ? data().kiloPass : null}>
                      {(pass) => (
                        ...
                        >
                          ...
                          >
                            <span style={{ "font-weight": "600", color: "var(--vscode-foreground)" }}>Blitx Pass</span>
                    ...
                    {/* No active Blitx Pass on the personal account — nudge to subscribe */}
