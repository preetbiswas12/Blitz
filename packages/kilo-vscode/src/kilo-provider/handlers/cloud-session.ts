/**
 * Cloud session handlers — extracted from BlitxProvider.
 *
...
import type { KiloClient, Session, TextPartInput, FilePartInput } from "@blitxcode/sdk/v2/client"
...
export async function handleRequestCloudSessions(
  ...
  try {
    ...
    console.error("[Blitx] BlitxProvider: Failed to fetch cloud sessions:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleRequestCloudSessionData(ctx: CloudSessionContext, sessionId: string): Promise<void> {
  ...
  try {
    ...
    console.error("[Blitx] Failed to load cloud session data:", err)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId: sessionId,
...
export async function handleImportAndSend(
  ...
  try {
    ...
    console.error("[Blitx] BlitxProvider: ❌ Cloud session import failed:", error)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId,
  ...
  try {
    ...
    console.error("[Blitx] Failed to send message after cloud import:", err)
    ctx.postMessage({
