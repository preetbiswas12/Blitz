...
import type { Event, Session } from "@blitxcode/sdk/v2/client"
...
describe("BlitxProvider follow-up sessions", () => {
  it("adopts pending follow-up sessions for single-session views", async () => {
    const service = connection()
    const provider = new BlitxProvider({} as never, service as never)
