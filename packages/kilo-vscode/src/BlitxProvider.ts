...
import { type BlitxConnectionService, ServerStartupError } from "./services/cli-backend"
...
import type { Agent } from "@blitxcode/sdk/v2/client"
import { configFeatures } from "./features"
import { createAutoApproveBridge } from "./kilo-provider/auto-approve"
import type { BlitxProviderOptions } from "./kilo-provider/options"
...
export class BlitxProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  public static readonly viewType = "blitx-code.SidebarProvider"
  ...
  private readonly extensionVersion =
    vscode.extensions.getExtension("kilocode.blitx-code")?.packageJSON?.version ?? "unknown"
  ...
  constructor(
    ...
    private readonly opts: BlitxProviderOptions = {},
  ) {
  ...
  private checkpoint(sid: string, run: () => Promise<void>): void {
    ...
    void pending.then(cleanup, (error) => {
      console.error("[Blitx] checkpoint mutation failed:", error)
      cleanup()
    })
  }
  ...
  private openMarketplacePanel(directory: unknown): void {
    if (typeof directory === "string" && directory) {
      vscode.commands.executeCommand("blitx-code.new.marketplaceButtonClicked", directory)
      return
    }
    vscode.commands.executeCommand("blitx-code.new.marketplaceButtonClicked", this.projectDirectory)
  }
  ...
  private async syncWebviewState(reason: string): Promise<void> {
    ...
    console.log("[Blitx] BlitxProvider: 🔄 syncWebviewState()", {
      reason,
    ...
    if (!this.isWebviewReady) {
      console.log("[Blitx] BlitxProvider: ⏭️ syncWebviewState skipped (webview not ready)")
      return
    }
    ...
    if (serverInfo) {
      const langConfig = vscode.workspace.getConfiguration("blitx-code.new")
      this.postMessage({
        type: "ready",
        serverInfo,
    ...
    if (this.connectionState === "connected" && this.client) {
      console.log("[Blitx] BlitxProvider: 👤 syncWebviewState fetching profile...")
      const profileResult = await retry(() => this.client!.kilo.profile())
      const profileData = profileResult.data ?? null
      console.log("[Blitx] BlitxProvider: 👤 syncWebviewState profile:", profileData ? "received" : "null")
      this.postMessage({
        type: "profileData",
        data: profileData,
      })
  ...
  private setSidebarVisible(visible: boolean): void {
    ...
    vscode.commands.executeCommand("setContext", "blitx-code.new.sidebarVisible", visible)
  }
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      if (
        await handleSidebarWorktreeMessage(message, {
          post: (msg) => this.postMessage(msg),
          openAgentManager: () => vscode.commands.executeCommand("blitx-code.new.agentManagerOpen"),
          openAdvancedWorktree: () => vscode.commands.executeCommand("blitx-code.new.agentManager.advancedWorktree"),
          openChanges: (sessionId?: string, turnId?: string) =>
            vscode.commands.executeCommand("blitx-code.new.showChanges", { sessionId, turnId }),
      ...
      switch (message.type) {
        case "webviewReady":
          console.log("[Blitx] BlitxProvider: ✅ webviewReady received")
        ...
        case "syncSession":
          this.handleSyncSession(message.sessionID, message.parentSessionID).catch((e) =>
            console.error("[Blitx] handleSyncSession failed:", e),
          )
          break
        case "loadSessions":
          this.handleLoadSessions().catch((e) => console.error("[Blitx] handleLoadSessions failed:", e))
          break
        ...
        case "openSettingsPanel":
          vscode.commands.executeCommand("blitx-code.new.settingsButtonClicked", message.tab)
          break
        ...
        case "forkSession":
          handleForkSession(this.forkCtx, message.sessionId, message.messageId).catch((e) =>
            console.error("[Blitx] handleForkSession failed:", e),
          )
          break
        case "retryConnection":
          console.log("[Blitx] BlitxProvider: 🔄 Retrying connection...")
          this.initializeConnection().catch((e) =>
            console.error("[Blitx] BlitxProvider: ❌ Retry connection failed:", e),
          )
          break
        case "openSubAgentViewer":
          vscode.commands.executeCommand("blitx-code.new.openSubAgentViewer", message.sessionID, message.title)
          break
        case "saveImage":
          return saveImage(this.getWorkspaceDirectory(this.currentSession?.id), message)
        case "requestProviders":
          this.fetchAndSendProviders().catch((e) => console.error("[Blitx] fetchAndSendProviders failed:", e))
          break
        ...
        case "fetchCustomProviderModels":
          this.handleFetchCustomProviderModels(message).catch((e) =>
            console.error("[Blitx] fetchCustomProviderModels failed:", e),
          )
          break
        case "compact":
          ...
          break
        case "requestAgents":
          this.fetchAndSendAgents().catch((e) => console.error("[Blitx] fetchAndSendAgents failed:", e))
          break
        case "requestSkills":
          this.fetchAndSendSkills().catch((e) => console.error("[Blitx] fetchAndSendSkills failed:", e))
          break
        case "requestAgentRequirements":
          this.requirements
            ...
            .catch((e) => console.error("[Blitx] fetchAndSendAgentRequirements failed:", e))
          break
        case "requestCommands":
          this.fetchAndSendCommands().catch((e) => console.error("[Blitx] fetchAndSendCommands failed:", e))
          break
        case "removeSkill":
          this.removeSkillViaCli(message.location).catch((e: unknown) =>
            console.error("[Blitx] removeSkill failed:", e),
          )
          break
        case "removeAgent":
          this.handleRemoveAgent(message.name).catch((e) => console.error("[Blitx] handleRemoveAgent failed:", e))
          break
        case "removeMcp":
          this.handleRemoveMcp(message.name).catch((e) => console.error("[Blitx] handleRemoveMcp failed:", e))
          break
        case "requestMcpStatus":
          this.fetchAndSendMcpStatus().catch((e) => console.error("[Blitx] fetchAndSendMcpStatus failed:", e))
          break
        case "connectMcp": {
          ...
          if (c1) {
            ...
            ).catch((e) => console.error("[Blitx] connectMcpServer failed:", e))
          }
          break
        }
        case "disconnectMcp": {
          ...
          if (c2) {
            void McpOAuth.disconnectMcpServer(c2, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Blitx] disconnectMcpServer failed:", e))
          }
          break
        }
        case "authenticateMcp": {
          ...
          if (c) {
            void McpOAuth.authenticateMcpServer(c, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Blitx] authenticateMcpServer failed:", e))
          }
          break
        }
        ...
        case "toggleSandbox":
          ...
          break
        case "requestConfig":
          this.fetchAndSendConfig().catch((e) => console.error("[Blitx] fetchAndSendConfig failed:", e))
          break
        case "requestGlobalConfig":
          this.fetchAndSendGlobalConfig().catch((e) => console.error("[Blitx] fetchAndSendGlobalConfig failed:", e))
          break
        case "requestIndexingStatus":
          this.fetchAndSendIndexingStatus().catch((e) =>
            console.error("[Blitx] fetchAndSendIndexingStatus failed:", e),
          )
          break
        ...
        case "openSettingsTab":
          if (message.tab === "indexing") {
            await vscode.commands.executeCommand("blitx-code.new.openIndexingSettings")
          }
          break
        case "setLanguage":
          await vscode.workspace
            .getConfiguration("blitx-code.new")
        ...
        case "requestRemoteStatus":
          this.remoteService
            ...
            .catch((err) => console.error("[Blitx] remote message failed:", err))
          break
        case "deleteSession":
        ...
        case "requestNotifications":
          this.fetchAndSendNotifications().catch((e) =>
            console.error("[Blitx] fetchAndSendNotifications failed:", e),
          )
          break
        ...
        case "enhancePrompt": {
          ...
          void sdkClient.enhancePrompt
            ...
            .catch((err: unknown) => {
              ...
              console.error("[Blitx] BlitxProvider: Failed to enhance prompt:", err)
   ...
   * Subscribes to the shared BlitxConnectionService.
   */
  ...
  private async doInitializeConnection(): Promise<void> {
    console.log("[Blitx] BlitxProvider: 🔧 Starting initializeConnection...")
    ...
    try {
      ...
      this.unsubscribeEvent = this.connectionService.onEventFiltered(
        (payload, directory) => {
          ...
          // BlitxProvider instance. The Settings panel is a separate provider with no tracked
      ...
      this.unsubscribeState = this.connectionService.onStateChange(async (state, error) => {
        ...
        if (state === "connected") {
          ...
          try {
            ...
            console.error("[Blitx] BlitxProvider: ❌ Failed during connected state handling:", error)
            this.postMessage({
              type: "error",
      ...
      // Subscribe to notification dismiss broadcast from other BlitxProvider instances
      this.unsubscribeNotificationDismiss = this.connectionService.onNotificationDismissed(() => {
        this.fetchAndSendNotifications()
      })

      // Subscribe to language change broadcast from other BlitxProvider instances
      ...
      // Subscribe to profile change broadcast from other BlitxProvider instances
      this.unsubscribeProfileChange = this.connectionService.onProfileChanged((data) => {
        this.postMessage({ type: "profileData", data })
      })

      // Subscribe to favorites change broadcast from other BlitxProvider instances
      this.unsubscribeFavoritesChange = this.connectionService.onFavoritesChanged((favorites) => {
        this.postMessage({ type: "favoritesLoaded", favorites })
      })

      // legacy-migration start
      // Subscribe to migration-complete broadcast from any BlitxProvider instance
      ...
      console.log("[Blitx] BlitxProvider: ✅ initializeConnection completed successfully")
    } catch (error) {
      console.error("[Blitx] BlitxProvider: ❌ Failed to initialize connection:", error)
      this.connectionState = "error"
      this.postMessage({
        type: "connectionState",
        state: "error",
  ...
  private async handleCreateSession(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to create session:", error)
      this.postMessage({
        type: "error",
  ...
  private refreshSessionDetails(sessionID: string, dir: string, signal?: AbortSignal): void {
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.warn("[Blitx] BlitxProvider: getSession failed (non-critical):", e))
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.error("[Blitx] BlitxProvider: Failed to fetch session statuses:", e))
  }
  ...
  private fetchAndSendSessionModelUsage(sessionID: string, requestID: string): Promise<void> {
    ...
    return this.connectionService
      ...
      .catch((error: unknown) => {
        console.warn("[Blitx] BlitxProvider: Failed to load session model usage:", error)
  ...
  private async handleLoadMessages(
    ...
    try {
      ...
      if (abort?.signal.aborted) return
      console.error("[Blitx] BlitxProvider: Failed to load messages:", error)
  ...
  private async handleSyncSession(sessionID: string, parentSessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to sync child session:", err)
    }
  }
  ...
  private async flushPendingSessionRefresh(reason: string): Promise<void> {
    ...
    console.log("[Blitx] BlitxProvider: 🔄 Flushing deferred sessions refresh", { reason })
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to flush session refresh:", error)
    }
  ...
  private async handleLoadSessions(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to load sessions:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleTerminalContext(requestId: string): Promise<void> {
    try {
      ...
      console.error("[Blitx] Failed to capture terminal context:", error)
      this.postMessage({
  ...
  private async handleDeleteSession(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to delete session:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleRenameSession(sessionID: string, title: string): Promise<void> {
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to rename session:", error)
  ...
  private async handleExportSessionTranscript(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to export session transcript:", error)
      this.postMessage({
        type: "error",
    ...
    const task = (async () => {
      ...
      while (true) {
        ...
        try {
          ...
          const settings = vscode.workspace.getConfiguration("blitx-code.new.model")
          const message = {
          ...
          console.error("[Blitx] BlitxProvider: Failed to fetch providers:", error)
        }
        if (!this.providersQueued) return
        generation = this.providersGeneration
      }
  ...
  private async fetchAndSendAgents(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch agents:", error)
    }
  }
  ...
  private async fetchAndSendSkills(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch skills:", error)
    }
  }
  ...
  private async fetchAndSendCommands(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch commands:", error)
    }
  }
  ...
  private async removeSkillViaCli(location: string): Promise<boolean> {
    ...
    try {
      ...
      if (result.error) {
        console.error("[Blitx] removeSkill returned error:", result.error)
      ...
      console.error("[Blitx] Failed to remove skill:", error)
      this.cachedSkillsMessage = null
      this.cachedCommandsMessage = null
      await Promise.all([this.fetchAndSendSkills(), this.fetchAndSendCommands()])
      return false
  ...
  private async handleRemoveAgent(name: string): Promise<void> {
    ...
    try {
      ...
      if (result.error) {
        console.error("[Blitx] removeAgent returned error:", result.error)
      }
    } catch (err) {
      console.error("[Blitx] Failed to remove agent:", err)
    }
    this.cachedAgentsMessage = null
  ...
  private async handleRemoveMcp(name: string): Promise<void> {
    ...
    if (!removed) {
      console.error("[Blitx] BlitxProvider: Failed to remove MCP server:", name)
    }
  }
  ...
  private async fetchAndSendMcpStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch MCP status:", error)
    }
  }
  ...
  private async fetchAndSendConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch config:", error)
    }
  }
  ...
  private async fetchAndSendGlobalConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch global config:", error)
    }
  }
  ...
  private async fetchAndSendIndexingStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch indexing status:", error)
    }
  }
  ...
  private async fetchAndSendConfigUpdated(): Promise<void> {
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to fetch config after update:", error)
    }
  }
  ...
  private async checkConfigWarnings(from: string): Promise<void> {
    if (this.configWarningsShown) {
      console.log("[Blitx] BlitxProvider: config warnings already shown", { from })
      return
    }
    if (!this.client) {
      console.log("[Blitx] BlitxProvider: config warnings skipped (no client)", { from })
      return
    }
    try {
      const dir = this.getWorkspaceDirectory()
      console.log("[Blitx] BlitxProvider: checking config warnings", { from, dir })
      const result = await this.client.config.warnings({ directory: dir })
      const list = result?.data ?? []
      console.log("[Blitx] BlitxProvider: config warnings fetched", { from, count: list.length })
      if (list.length === 0) return
      this.configWarningsShown = true

      const first = list[0]!
      const summary = list.length === 1 ? first.message : `${first.message} (and ${list.length - 1} more)`
      console.warn("[Blitx] BlitxProvider: showing config warnings", { from, count: list.length, path: first.path })
      ...
      if (action === "Show Details") {
        ...
        const channel = vscode.window.createOutputChannel("Blitx Config Warnings")
        channel.clear()
        channel.appendLine(lines.join("\n\n"))
        channel.show()
      }
    } catch (err) {
      console.warn("[Blitx] BlitxProvider: checkConfigWarnings failed:", { from, err })
    }
  }
  ...
  private sendNotificationSettings(): void {
    const attention = vscode.workspace.getConfiguration("blitx-code.new.attention")
    this.postMessage({
  ...
  private sendTimelineSetting(): void {
    const config = vscode.workspace.getConfiguration("blitx-code.new")
    this.postMessage({
  ...
  private async runToggleSandbox(
    ...
    try {
      ...
      const remembered = await this.connectionService.sandboxPreference
        ...
        .then(() => true)
        .catch((error) => {
          console.error("[Blitx] Failed to persist sandbox default:", error)
          return false
        })
      this.postMessage({
        type: "sandboxStatus",
  ...
  private async handleUpdateConfig(
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Config write succeeded but post-write refresh failed:", error)
      const patch =
  ...
  private postConfigFailure(error: unknown): void {
    console.error("[Blitx] BlitxProvider: Failed to update config:", error)
    this.postMessage({
  ...
  private async withRetry(
    ...
    try {
      for (let attempt = 1; ; attempt++) {
        ...
        console.log(`[Blitx] BlitxProvider: Retry on ${status}, attempt ${attempt}/${MAX_RETRIES}, delay ${delay}ms`)
  ...
  private maxCostSetting(): number {
    return this.setMaxCost(vscode.workspace.getConfiguration("blitx-code.new").get<number>("maxCost", 0))
  }
  ...
  private async handleSendMessage(
    ...
    try {
      ...
      await runWithMessageConfirmation(this.confirmations, messageID, "BlitxProvider: Message request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Blitx] BlitxProvider: Failed to send message:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleSendCommand(
    ...
    try {
      ...
      await this.requirements.assertAgentRequirements(agent, dir)
      await this.checkpoints.get(sid)
      await runWithMessageConfirmation(this.confirmations, messageID, "BlitxProvider: Command request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Blitx] BlitxProvider: Failed to send command:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleRevertSession(sessionID: string, messageID: string, partID?: string): Promise<void> {
    ...
    if (error) {
      console.error("[Blitx] BlitxProvider: Failed to revert session:", error)
  ...
  private async handleUnrevertSession(sessionID: string): Promise<void> {
    ...
    if (error) {
      console.error("[Blitx] BlitxProvider: Failed to unrevert session:", error)
  ...
  private async handleCompact(sessionID?: string, providerID?: string, modelID?: string): Promise<void> {
    ...
    if (!target) {
      console.error("[Blitx] BlitxProvider: No sessionID for compact")
      return
    }

    if (!providerID || !modelID) {
      console.error("[Blitx] BlitxProvider: No model selected for compact")
      this.postMessage({
        type: "error",
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to compact session:", error)
      this.postMessage({
        type: "error",
  ...
  private async disposeGlobal(): Promise<void> {
    ...
    await this.client.global
      .dispose()
      .catch((e: unknown) => console.warn("[Blitx] BlitxProvider: global.dispose() after org switch failed:", e))
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to refresh profile after org switch:", error)
    }
    try {
      await this.fetchAndSendProviders()
    } catch (error) {
      console.error("[Blitx] BlitxProvider: Failed to refresh providers after org switch:", error)
    }
  }

  /**
   * Handle a generic setting update from the webview.
   * The key uses dot notation relative to `blitx-code.new` (e.g. "browserAutomation.enabled").
   */
  private async handleUpdateSetting(key: string, value: unknown): Promise<void> {
    ...
    const config = vscode.workspace.getConfiguration(`blitx-code.new${section ? `.${section}` : ""}`)
  ...
  /**
   * Reset all "blitx-code.new.*" extension settings to their defaults by reading
   * contributes.configuration from the extension's package.json at runtime.
   * Only resets settings under the "blitx-code.new." namespace to avoid touching
   * settings from the previous version of the extension which shares the same
   * extension ID and "blitx-code.*" namespace.
   */
  private async handleResetAllSettings(): Promise<void> {
    const confirmed = await vscode.window.showWarningMessage(
      "Reset all Blitx extension settings to defaults?",
      { modal: true },
      "Reset",
    )
    if (confirmed !== "Reset") return

    const prefix = "blitx-code.new."
    const ext = vscode.extensions.getExtension("kilocode.blitx-code")
    ...
    vscode.window.showInformationMessage("Blitx settings have been reset to defaults.")
  }
  ...
  private sendBrowserSettings(): void {
    const config = vscode.workspace.getConfiguration("blitx-code.new.browserAutomation")
    this.postMessage({
  ...
  private sendClaudeCompatSetting(): void {
    const enabled = vscode.workspace.getConfiguration("blitx-code.new").get<boolean>("claudeCodeCompat", false)
    this.postMessage({
  ...
  private handleEvent(event: ProviderEvent, directory?: string): void {
    ...
    // so this runs on every BlitxProvider instance — including the Settings panel which has no
    ...
    if (event.type === "message.part.updated") {
      ...
      if (childId && !this.trackedSessionIds.has(childId)) {
        console.log("[Blitx] BlitxProvider: 🔗 Auto-adopting child session from task tool", { childId })
    ...
    if (!this.webview) {
      const type =
        ...
        typeof (message as { type?: unknown }).type === "string"
          ...
          : "<unknown>"
      console.warn("[Blitx] BlitxProvider: ⚠️ postMessage dropped (no webview)", { type })
      return
    }

    void this.webview.postMessage(message).then(undefined, (error) => {
      console.error("[Blitx] BlitxProvider: ❌ postMessage failed", error)
    })
  }
  ...
  public async appendReviewComments(comments: unknown[], autoSend = false): Promise<void> {
    ...
    if (!this.webview) {
      await vscode.commands.executeCommand(`${BlitxProvider.viewType}.focus`)
    }
  ...
  private async getGitRemoteUrl(): Promise<string | undefined> {
    try {
      ...
      console.warn("[Blitx] BlitxProvider: Failed to get git remote URL:", error)
      return undefined
    }
  }
