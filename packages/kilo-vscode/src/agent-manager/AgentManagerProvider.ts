...
import type { KiloClient, Session } from "@blitxcode/sdk/v2/client"
...
export class AgentManagerProvider implements Disposable {
  public static readonly viewType = "blitx-code.new.AgentManagerPanel"
  ...
  constructor(
    ...
    this.outputChannel = host.createOutput("Blitx Agent Manager")
  ...
  private openWorktreeFile(sessionId: string, filePath: string, line?: number, column?: number): void {
    ...
    try {
      ...
      console.error("[Blitx] AgentManagerProvider: Cannot resolve file path:", err)
      return
    }
   ...
   * Called from BlitxProvider when the sidebar sends "continueInWorktree".
   */
