 ...
 * ## Why this exists
 *
 * The sidebar BlitxProvider always uses `getWorkspaceDirectory()`, which returns
 ...
 *
 * ## How BlitxProvider uses this
 *
 * Each BlitxProvider instance can receive an explicit `projectDirectory` via
 * `BlitxProviderOptions`. When set:
 *
...
/**
 * Resolve the effective project directory for a BlitxProvider instance.
 *
 * @param override - Explicit directory from BlitxProviderOptions. `undefined`
 ...
 *   `getWorkspaceDirectory(sessionId)` from BlitxProvider).
 */
