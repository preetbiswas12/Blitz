...
import type { ConnectionState, BlitxConnectionService } from "../cli-backend/connection-service"
...
export class NotebookBridge {
  ...
  constructor(
    ...
    this.unsubscribeState = connection.onStateChange((state) => {
      ...
      void this.recover(revision).catch((error: unknown) => {
        console.error("[Blitx] NotebookBridge: pending request recovery failed:", error)
      })
    })
  }
  ...
  dispose(): void {
    ...
    for (const context of this.contexts.values()) {
      void context
        ...
        .catch((error: unknown) => console.error("[Blitx] NotebookBridge: context disposal failed:", error))
    }
    this.contexts.clear()
    this.origins.clear()
    this.outcomes.clear()
  ...
  private start(request: NotebookRequest, origin: RequestOrigin): void {
    ...
    void this.run(request, origin, active).catch((error: unknown) => {
      console.error(`[Blitx] NotebookBridge: request ${request.id} failed:`, error)
    })
  }
  ...
  private async reply(requestID: string, directory: string, result: NotebookResult): Promise<boolean> {
    try {
      ...
      console.error(`[Blitx] NotebookBridge: reply ${requestID} failed:`, response.error)
      return false
    } catch (error) {
      console.error(`[Blitx] NotebookBridge: reply ${requestID} failed:`, error)
      return false
    }
  }
  ...
  private async reject(requestID: string, directory: string, error: NotebookFailure): Promise<boolean> {
    try {
      ...
      if (!response.error) return true
      console.error(`[Blitx] NotebookBridge: rejection ${requestID} failed:`, response.error)
      return false
    } catch (cause) {
      console.error(`[Blitx] NotebookBridge: rejection ${requestID} failed:`, cause)
      return false
    }
  }
  ...
  private async recover(revision: number): Promise<void> {
    ...
    for (const directory of this.connection.getKnownDirectories()) {
      try {
        ...
        if (response.error) {
          console.error(`[Blitx] NotebookBridge: could not list requests for ${directory}:`, response.error)
          continue
        }
        ...
        console.error(`[Blitx] NotebookBridge: could not list requests for ${directory}:`, error)
      }
    }
  }
...
export function createNotebookBridge(connection: BlitxConnectionService): NotebookBridge {
