...
Blitx is an open source AI coding agent platform. It ships as a CLI and editor clients that all build on the same backend. This package (`packages/kilo-vscode/`) is the **VS Code extension**.

### Products and How They Relate

All products are thin clients over the **CLI** (`packages/opencode/`, published as `@blitxcode/cli`). The CLI is a fork of upstream [OpenCode](https://github.com/anomalyco/opencode) with Kilo-specific additions (gateway auth, telemetry, migration, code review, branding). It contains the full AI agent runtime, tool execution, session management, provider integrations (500+ models), and an HTTP API server.

Every client spawns or connects to a `blitx serve` process and communicates via HTTP REST + SSE using the auto-generated `@blitxcode/sdk`.

```
                        @blitxcode/cli  (packages/opencode/)
                     ┌────────────────────────────────┐
...
|---|---|---|---|
| Blitx CLI (TUI) | `packages/opencode/` | Interactive terminal UI (SolidJS + OpenTUI) | In-process — TUI and server run together |
| Blitx CLI (`blitx run`) | `packages/opencode/` | Non-interactive headless mode for scripting | In-process — no network socket |
| **Blitx VS Code Extension** | **`packages/kilo-vscode/`** | VS Code extension with sidebar chat + Agent Manager | Bundles CLI binary, spawns `blitx serve --port 0` as child process |
...
| `packages/kilo-gateway/` | `@blitxcode/kilo-gateway` | Auth (device flow), AI provider routing (OpenRouter), Kilo API integration (profile, balance, teams) |
| `packages/kilo-ui/` | `@blitxcode/kilo-ui` | SolidJS component library (40+ components, built on `@kobalte/core`). Shared by this extension's webview and docs screenshot stories |
| `packages/kilo-telemetry/` | `@blitxcode/kilo-telemetry` | PostHog analytics + OpenTelemetry tracing for the CLI |
| `packages/kilo-i18n/` | `@blitxcode/kilo-i18n` | Translation strings (16 languages) |
| `packages/kilo-docs/` | `@blitxcode/kilo-docs` | Documentation site (Next.js + Markdoc) |

### Upstream OpenCode Packages (not Kilo-specific)

| Package | Name | Role |
|---|---|---|
| `packages/opencode/` | `@blitxcode/cli` | Core CLI — forked from upstream OpenCode. AI agents, tools, sessions, server. |
...
The extension bundles its own CLI binary at `bin/blitx` — it does NOT use a system-installed CLI. To build it:
...
The script checks for a prebuilt binary in `packages/opencode/dist/`, builds the CLI if needed, and copies it to `bin/blitx`.

## Architecture

### Extension ↔ CLI Backend

The extension is a client of the CLI. Activation creates one shared `KiloConnectionService`; on its first connection, which autocomplete may prewarm, `ServerManager` spawns `bin/blitx serve --port 0`, captures the dynamically assigned port from stdout, and communicates over HTTP + SSE. The current child process is reused unless it exits. A random password is generated and passed via `BLITX_SERVER_PASSWORD` env var for basic auth.

```
Extension (Node.js)                          CLI Backend (child process)
┌──────────────────────────┐                ┌──────────────────────┐
│ KiloConnectionService    │── HTTP/SSE ──> │ blitx serve --port 0  │
...
Agent Manager local worktree sessions use the current shared `blitx serve` process owned by `KiloConnectionService`; no session starts its own backend. Their CLI requests pass the worktree path as `directory`, which resolves directory-scoped backend state. Setup scripts, terminal PTYs, git subprocesses, and a separately opened VS Code window are separate process or extension-host boundaries, not per-worktree `blitx serve` instances.
...
New webview features must use **`@blitxcode/kilo-ui`** components instead of raw HTML elements with inline styles. This is a Solid.js component library built on `@kobalte/core`.

- Import via deep subpaths: `import { Button } from "@blitxcode/kilo-ui/button"`
...
- Global styles imported via `import "@blitxcode/kilo-ui/styles"` in [`index.tsx`](webview-ui/src/index.tsx:2)
...
The `spawn` wrapper covers long-lived processes (e.g. `blitx serve`). The `exec` wrapper covers short commands (e.g. `git`, `tar`). If you need the raw callback form of `execFile` for some reason, pass `windowsHide: true` explicitly in the options object.
