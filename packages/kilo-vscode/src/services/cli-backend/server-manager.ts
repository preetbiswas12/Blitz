...
export function resolveIndexingEnv(folders: readonly WorkspaceFolderLike[] | undefined): Record<string, string> {
  ...
  return { BLITX_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" }
}

export function resolveManagedServerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return { ...env, BLITX_DISABLE_CHANNEL_DB: "true" }
}
...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    console.log("[Blitx] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[Blitx] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[Blitx] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[Blitx] ServerManager: 🚀 Starting new server instance...")
    this.startupPromise = this.startServer()
    try {
      this.instance = await this.startupPromise
      console.log("[Blitx] ServerManager: ✅ Server started successfully:", { port: this.instance.port })
      return this.instance
    } finally {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[Blitx] ServerManager: 📍 CLI path:", cliPath)
    console.log("[Blitx] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    console.log("[Blitx] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[Blitx] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))

    return new Promise((resolve, reject) => {
      console.log("[Blitx] ServerManager: 🎬 Spawning CLI process:", cliPath, ["serve", "--port", "0"])
      const cfg = vscode.workspace.getConfiguration("blitx-code.new")
      ...
      //     `blitx-code.new.extraCaCerts` (NODE_EXTRA_CA_CERTS).
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          ...(!claudeCompat && { BLITX_DISABLE_CLAUDE_CODE: "true" }),
      ...
      console.log("[Blitx] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[Blitx] ServerManager: 📥 CLI Server stdout:", output)

        const port = parseServerPort(output)
        if (port !== null && !resolved) {
          resolved = true
          console.log("[Blitx] ServerManager: 🎯 Port detected:", port)
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[Blitx] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
        stderrLines.push(errorOutput)
      })

      serverProcess.on("error", (error) => {
        console.error("[Blitx] ServerManager: ❌ Process error:", error)
        if (!resolved) {
          reject(error)
        }
      })
      ...
      serverProcess.on("exit", (code) => {
        console.log("[Blitx] ServerManager: 🛑 Process exited with code:", code)
      ...
      setTimeout(() => {
        if (!resolved) {
          console.error(`[Blitx] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
  ...
  private getCliPath(): string {
    ...
    console.log("[Blitx] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  dispose(): void {
    ...
    console.log("[Blitx] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[Blitx] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
