import type { KiloClient, GlobalEvent } from "@blitxcode/sdk/v2/client"
...
export class SdkSSEAdapter {
  ...
  connect(): void {
    if (this.abortController) {
      console.log("[Blitx] SSE: ⚠️ Already connected, skipping")
      return
    }

    console.log("[Blitx] SSE: 🔌 connect() called")
    this.abortController = new AbortController()
    console.log('[Blitx] SSE: 🔄 Setting state to "connecting"')
    this.notifyState("connecting")
    void this.consumeLoop(this.abortController.signal).catch((err) => {
      console.error("[Blitx] SSE: Unhandled error in consumeLoop:", err)
   ...
   */
  disconnect(): void {
    console.log("[Blitx] SSE: 🔌 disconnect() called")
  ...
  reconnect(): void {
    if (!this.attemptController) {
      console.log("[Blitx] SSE: ⚠️ reconnect() called but no active attempt")
      return
    }
    console.log("[Blitx] SSE: 🔄 reconnect() — aborting current attempt")
  ...
  private async consumeLoop(signal: AbortSignal): Promise<void> {
    ...
    while (!signal.aborted) {
      ...
      try {
        console.log("[Blitx] SSE: 🎬 Calling SDK global.event()...")
        const events = await this.client.global.event({
          ...
          onSseError: (error) => {
            ...
            console.error("[Blitx] SSE: ❌ SDK SSE error callback:", error)
            this.notifyError(error instanceof Error ? error : new Error(String(error)))
          },
        })

        console.log("[Blitx] SSE: ⏳ Waiting for first stream event")
        ...
        for await (const event of events.stream) {
          ...
          if (!ready) {
            ...
            console.log("[Blitx] SSE: ✅ Stream opened successfully")
        ...
        console.log(
          ready ? "[Blitx] SSE: 📭 Stream ended normally" : "[Blitx] SSE: 📭 Stream ended before first event",
        )
      } catch (error) {
        ...
        if (!aborted) {
          console.error("[Blitx] SSE: ❌ Stream error:", error)
          this.notifyError(error instanceof Error ? error : new Error(String(error)))
        }
      } finally {
      ...
      console.log(`[Blitx] SSE: 🔄 Reconnecting in ${wait}ms...`)
      this.notifyState("connecting")
  ...
  private resetHeartbeat(attempt: AbortController): void {
    ...
    this.heartbeatTimer = setTimeout(() => {
      console.log("[Blitx] SSE: ⏰ Heartbeat timeout — aborting stale connection")
      attempt.abort()
  ...
  private notifyEvent(event: SSEPayload, directory?: string): void {
    for (const handler of this.handlers) {
      try {
        ...
        console.error("[Blitx] SSE: Error in event handler:", error)
      }
    }
  }
  ...
  private notifyError(error: Error): void {
    for (const handler of this.errorHandlers) {
      try {
        ...
        console.error("[Blitx] SSE: Error in error handler:", err)
      }
    }
  }
  ...
  private notifyState(state: "connecting" | "connected" | "disconnected"): void {
    for (const handler of this.stateHandlers) {
      try {
        ...
        console.error("[Blitx] SSE: Error in state handler:", error)
      }
    }
  }
}
