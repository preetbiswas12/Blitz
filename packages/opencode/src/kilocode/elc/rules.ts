...
Configure in \`~/.legion/settings.json\`:
...
- Configure \`allowedTools\` in \`~/.legion.json\` instead
...
- **Config**: Set \`alwaysThinkingEnabled\` in \`~/.legion/settings.json\`
...
> This file covers **React hooks** (\`useState\`, \`useEffect\`, \`useMemo\`, \`useCallback\`, custom hooks) — NOT the Legion CLI \`hooks/\` runtime system. Naming matches the per-language convention \`rules/<lang>/hooks.md\` used across this repo.
>
...
export const ELC_RULES: ElcRule[] = [
