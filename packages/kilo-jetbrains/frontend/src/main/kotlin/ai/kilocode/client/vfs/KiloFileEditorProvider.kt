...
class KiloFileEditorProvider : FileEditorProvider, DumbAware {
    ...
    override fun createEditor(project: Project, file: VirtualFile): FileEditor {
        ensureAttachmentEditorKind()
        val path = path(file) ?: error("Invalid Blitx virtual file: ${file.path}")
        val kilo = file as? KiloVirtualFile ?: KiloVirtualFile(path)
        val kind = service<KiloEditorKindRegistry>().get(kilo.path.kind) ?: error("Unknown Blitx editor kind: ${kilo.path.kind}")
