...
describe("server workspace helpers", () => {
  ...
  it("disables codebase indexing only when no workspace folder is open", () => {
    expect(resolveIndexingEnv(undefined)).toEqual({ BLITX_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
    expect(resolveIndexingEnv([])).toEqual({ BLITX_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
  ...
  it("uses the shared database for the managed backend while preserving the environment", () => {
    expect(resolveManagedServerEnv({ PATH: "/usr/bin", BLITX_DISABLE_CHANNEL_DB: "false" })).toEqual({
      PATH: "/usr/bin",
