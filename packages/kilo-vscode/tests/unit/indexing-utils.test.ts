...
import type { EventIndexingStatus, IndexingStatus } from "@blitxcode/sdk/v2/client"
...
describe("indexing feature detection", () => {
  ...
  it("detects supported indexing plugin specifiers", () => {
    ...
    expect(configFeatures({ plugin: ["@blitxcode/kilo-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@blitxcode/kilo-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/.opencode/plugin/kilo-indexing.js"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/node_modules/@blitxcode/kilo-indexing/index.js"] }).indexing).toBe(
      true,
    )
  })
