/**
 * Authentication handlers — extracted from BlitxProvider.
 *
...
export async function handleLogin(ctx: AuthContext, attempt: number, getAttempt: () => number): Promise<void> {
  ...
  console.log("[Blitx] BlitxProvider: 🔐 Starting login flow...")
  ...
  try {
    ...
    const { data: auth } = await ctx.client.provider.oauth.authorize(
      { providerID: "blitx", method: 0, directory: dir },
      { throwOnError: true },
    )
    console.log("[Blitx] BlitxProvider: 🔐 Got auth URL:", auth.url)
    ...
    await ctx.client.provider.oauth.callback({ providerID: "blitx", method: 0, directory: dir }, { throwOnError: true })

    // Check if this attempt was cancelled
    if (attempt !== getAttempt()) return

    console.log("[Blitx] BlitxProvider: 🔐 Login successful")
...
export async function handleLogout(ctx: AuthContext): Promise<void> {
  ...
  try {
    console.log("[Blitx] BlitxProvider: 🚪 Logging out...")
    await ctx.client.auth.remove({ providerID: "blitx" }, { throwOnError: true })
    console.log("[Blitx] BlitxProvider: 🚪 Logged out successfully")
    ...
    console.error("[Blitx] BlitxProvider: ❌ Logout failed:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleSetOrganization(ctx: AuthContext, organizationId: string | null): Promise<void> {
  ...
  console.log("[Blitx] BlitxProvider: Switching organization:", organizationId ?? "personal")
  try {
    await ctx.client.kilo.organization.set({ organizationId }, { throwOnError: true })
  } catch (error) {
    console.error("[Blitx] BlitxProvider: Failed to switch organization:", error)
    ...
    try {
      ...
      console.error("[Blitx] BlitxProvider: Failed to refresh profile after org switch error:", profileError)
    }
    return
  }
  ...
  try {
    ...
    console.error("[Blitx] BlitxProvider: Failed to refresh agents after org switch:", error)
  }
}
...
export async function handleRefreshProfile(ctx: AuthContext): Promise<void> {
  ...
  console.log("[Blitx] BlitxProvider: 🔄 Refreshing profile...")
